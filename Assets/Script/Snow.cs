...
public class Snow : MonoBehaviour {
	...
	void Start () {
		...
		for (var i = 0; i < SNOW_NUM; ++i) {
			float x = Random.Range (-rangeR_, rangeR_);
			float y = Random.Range (-rangeR_, rangeR_);
			float z = Random.Range (-rangeR_, rangeR_);
	...
	void LateUpdate(){
		var target_position = Camera.main.transform.TransformPoint (Vector3.forward * rangeR_);
		var mr = GetComponent<Renderer> ();
		mr.material.SetFloat ("_Range", range_);
		mr.material.SetFloat ("RangeR_", rangeR_);
